...
If packaging, use the `rootdir` variable to change the root path, in addition to the prefix: `just rootdir=debian/lingmo-background prefix=/usr install`.
...
To get debug logs from the service, first kill the `lingmo-background` process a few times in a row to prevent it from being launched by `lingmo-session`. Then launch it with `just run` to display backtraces and debug logs in the terminal.
